 ...
 */
LinkedListNode *linkedListNodeCreate(TreeNodePtr value) {
 ...
 */
TreeNodePtr linkedListGet(LinkedList **head, int position) {
  LinkedListNode *current = *head;
...
void displayLinkedList(LinkedList *head) {
  ...
  for (int i = 0; ptrActual != NULL; i++) {
    printf("[%d: %p]->", i, (void*)ptrActual->data);
